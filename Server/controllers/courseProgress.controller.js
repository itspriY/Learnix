...
export const getCourseProgress = async (req,res) => {
    try{
        ...
        const userId = req.id;
         ...
         //step-3 return the users course progress along with the course Details
...
export const updateLectureProgress = async(req,res) => {
    try {
       ...
       if(!courseProgress){
        //If no course exist ,create a new record
...
export const markAsCompleted = async (req,res) => {
    try {
        ...
        if(!courseProgress) return res.status(400).json({mesaage:"Course progress not found"});
